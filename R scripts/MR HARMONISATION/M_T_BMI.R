...
M_T_proxies_output <- read_excel("not found inputs/SNPs_M_Testosterone_AND_BMI.xlsx", sheet = "T&P E&O")



allele_matching <- select(M_T_proxies_output, "SNP", "ALLELE1", "ALLELE0", "A1FREQ", "BETA.x", "SE.x", "Tested_Allele", "Other_Allele", "BETA.y", "SE.y" )
...
allele_matching <- allele_matching %>%
  rename(
    ...
    BETA_T = "BETA.x",
    SE_T = "SE.x",
    Reference_allele_BMI = "Other_Allele",
    Effect_allele_BMI = "Tested_Allele",
    beta_BMI = "BETA.y",
    se_BMI = "SE.y"
  )
...
allele_matching$beta_BMI <- as.numeric(allele_matching$beta_BMI)

allele_matching$HARM_MALE_BETA_BMI <- if_else(allele_matching$T_inc_allele!=allele_matching$Effect_allele_BMI,
                                              allele_matching$beta_BMI*-1, allele_matching$beta_BMI)




plot(allele_matching$ABS_BETA_T, allele_matching$HARM_MALE_BETA_BMI)
allele_matching$se_BMI <- as.numeric(allele_matching$se_BMI)
IVW_weights <- allele_matching$se_BMI^-2 
inverse_weighted_LR <- lm(allele_matching$HARM_MALE_BETA_BMI ~ allele_matching$ABS_BETA_T- 1 ,weights=IVW_weights)
...
allele_matching$HARM_MALE_BETA_CAD <- if_else(allele_matching$T_inc_allele!=allele_matching$other_allele_CAD,
