...
F_SHBG_proxies_output <- read_excel("not found inputs/SNPs_F_SHBG_AND_DBP.xlsx", sheet = "T&P E&O")
F_SHBG_proxies_output <- F_SHBG_proxies_output[-1,]


allele_matching <- select(F_SHBG_proxies_output, "CHR", "BP", "SNP.x", "ALLELE1", "ALLELE0", "A1FREQ", "BETA", "SE", "Allele1", "Allele2", "Freq1", "Effect", "StdError" )
...
allele_matching$HARM_BETA_DBP <- if_else(allele_matching$SHBG_inc_allele!=allele_matching$other_allele_DBP,
                                         allele_matching$beta_DBP*-1, allele_matching$beta_DBP)




plot(allele_matching$ABS_BETA_SHBG, allele_matching$HARM_BETA_DBP)


F_SHBG_proxies_output$StdError <- as.numeric(F_SHBG_proxies_output$StdError)
IVW_weights <- F_SHBG_proxies_output$StdError^-2 
inverse_weighted_LR <- lm(allele_matching$HARM_BETA_DBP ~ allele_matching$ABS_BETA_SHBG- 1 ,weights=IVW_weights)
...
MRObject = mr_input(bx = allele_matching$ABS_BETA_SHBG, bxse = allele_matching$SE_SHBG, 
                    by = allele_matching$HARM_BETA_DBP, byse = allele_matching$se_DBP, snps = allele_matching$SNP_SHBG)
